...
public class ClawIOCTRE implements ClawIO {
  ...
  private ArmFeedforward wristFF = new ArmFeedforward(kS, kG, kV, kA);
    ...
    public void updateInputs(ClawIOInputs inputs) {
      ...
      if (locked) {
        wrist.setVoltage(wristPID.calculate(inputs.angle.magnitude())+wristFF.calculate(inputs.angle.magnitude(), wristPID.getGoal().velocity));
      }
    ...
    private void tempPIDTuning() {
      ...
      if (kS != SmartDashboard.getNumber("Claw/PID/S", kS)) {
        kS = SmartDashboard.getNumber("Claw/PID/S", kS);
        wristFF = new ArmFeedforward(kS, kG, kV, kA);
    }

    if (kG != SmartDashboard.getNumber("Claw/PID/G", kG)) {
      kG = SmartDashboard.getNumber("Claw/PID/G", kG);
      wristFF = new ArmFeedforward(kS, kG, kV, kA);
    }
    if (kV != SmartDashboard.getNumber("Claw/PID/V", kV)) {
      kV = SmartDashboard.getNumber("Claw/PID/V", kV);
      wristFF = new ArmFeedforward(kS, kG, kV, kA);
    }

    if (kA != SmartDashboard.getNumber("Claw/PID/A", kA)) {
      kA = SmartDashboard.getNumber("Claw/PID/A", kA);
      wristFF = new ArmFeedforward(kS, kG, kV, kA);
    }

    if (kacel != SmartDashboard.getNumber("Claw/PID/Acel", kacel)) {
      kacel = SmartDashboard.getNumber("Claw/PID/Acel", kacel);
      wristPID.setConstraints(new TrapezoidProfile.Constraints(kvel, kacel));
    }

    if (kvel != SmartDashboard.getNumber("Claw/PID/Vel", kvel)) {
      kvel = SmartDashboard.getNumber("Claw/PID/Vel", kvel);
      wristPID.setConstraints(new TrapezoidProfile.Constraints(kvel, kacel));
    }
  }
}
