...
public class NoteVisualizer {
  ...
  public static Command loadCoral(Drive drivetrain) {
    final Timer timer = new Timer();
    timer.start();
    final Pose3d onRobot = new Pose3d(robotPoseSupplier.get());
    final Pose3d source = new Pose3d(0, 0, 0, null);

    if (drivetrain.getChassisSpeeds().vxMetersPerSecond == 0 && timer.get() > 0.5) {
      return Commands.run(
          () -> {
            Logger.recordOutput("NoteVisualizer", onRobot);
          },
          drivetrain);
    } else {
      return Commands.run(
          () -> {
            Logger.recordOutput("NoteVisualizer", source);
          },
          drivetrain);
    }
  }
